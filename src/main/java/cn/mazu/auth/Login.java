...
public class Login extends WObject {
	...
	*//**
	...
	public void login(User user, LoginState state) {
		...
		//临时cookie
		AuthInfo ai = user.getDatabase().entityManager_.find(AuthInfo.class, user.getId());
		WApplication.getInstance().setCookie("loginid", ai.getUser().getId(),0);
		WApplication.getInstance().setCookie("username",ai.getAuthIdentities().get(0).getIdentity(),0);
