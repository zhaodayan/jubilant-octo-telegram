...
public class AuthService {
	 ...
	 */
	/*public synchronized User identifyUser(Identity identity, UserDatabase users) {
		UserDatabase.Transaction t = users.startTransaction();
	 ...
	 * in
	 * {@link AuthService#processAuthToken(String token, UserDatabase users)
	 * processAuthToken()}.
	 */
	/*public synchronized String createAuthToken(User user) {
		if (!user.isValid()) {
			throw new WException("Auth: createAuthToken(): user invalid");
		}
		UserDatabase.Transaction t = user.getDatabase()
				.startTransaction();
	 ...
	 */
	/*public synchronized AuthTokenResult processAuthToken(String token,
			UserDatabase users) {
		UserDatabase.Transaction t = users.startTransaction();
	 ...
	 * <p>
	 * 
	 * @see AuthService#processEmailToken(String token, UserDatabase
	 *      users)
	 */
	 ...
	 *      users)
	 */
	/*public void lostPassword(String emailAddress, UserDatabase users)
			throws javax.mail.MessagingException, UnsupportedEncodingException,
			IOException {
	 ...
	 * @see AuthService#lostPassword(String emailAddress, UserDatabase
	 *      users)
	 */
	/*public synchronized EmailTokenResult processEmailToken(String token,
			UserDatabase users) {
		UserDatabase.Transaction tr = users.startTransaction();
		String hash = this.getTokenHashFunction().compute(token, "");
	 ...
	 * configured using the smtp.host and smpt.port mazu configuration variables
