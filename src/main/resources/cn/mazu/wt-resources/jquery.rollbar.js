	...
	*/
	...
	(function($){
		$.fn.rollbar=function(s){
				var a={
						scroll:'both',autoHide:true,autoHideTime:'fast',lazyCheckScroll:1000,blockGlobalScroll:false,
						contentFilter:'*',sliderSize:'60%',sliderOpacity:0.5,sliderOpacityTime:200,
						sliderOpacityDelay:1000,wheelSpeed:20,touchSpeed:0.3,pathPadding:'20px',
						keyScroll:100,scrollTime:500,scrollInterval:15,scrollEasing:'swing',zIndex:100,
						onscroll:function(){}
				};
				$.extend(a,s);
		...
		function RollBar(c,s){
			...
			this.container.css({'position':'relative','overflow':'hidden'}).contents()
				.filter(this.settings.contentFilter).wrapAll('<div class="rollbar-content"></div>');
			this.content=this.container.children('.rollbar-content').css({'position':'relative','top':0,'left':0,'overflow':'hidden'});
			this.content.width(this.content.contents().width());
			this.content.height(this.content.contents().height());
		...
		RollBar.prototype.pathSize=function(){
			...
			RollBar.prototype.easeScroll=function(v,h){
				...
				var d=$.easing[this.settings.scrollEasing]||$.easing.linear;this.sliders.stop().fadeTo(this.settings.sliderOpacityTime,1);
			...
			RollBar.prototype.init=function(){
				...
				//this.bindEvent(this.container,'mousedown',function(e){
					...
					//this.bindEvent(this.sliders,'mousemove',function(e){
						if(this.pressed==1){
					...
					//this.bindEvent(this.sliders,'selectstart',function(e){
						e.preventDefault()
					})
				});
				/*this.bindEvent(this.sliders,'mouseup',function(e){
					...
					this.pressed=0;
					this.sliders.unbind('mousemove');
					this.sliders.unbind('selectstart')
				});*/
				...
				this.bindEvent(this.container,'touchmove',function(e){
					...
					this.scroll(this.touch.sv+(this.touch.sy-b.pageY)*this.settings.touchSpeed,
							this.touch.sh+(this.touch.sx-b.pageX)*this.settings.touchSpeed,e);
					if(this.settings.blockGlobalScroll&&(this.vdiff||this.hdiff)){
						a.preventDefault();
						a.stopPropagation()
					}
				...
				this.bindEvent($(window),'resize',function(){
					...
					this.scroll(Math.round(parseInt(this.vslider.css('top'),10)*this.vpath.height()/g),
				...
				this.bindEvent(this.container,'mousewheel',function(e,a,b,c){
					...
					this.scroll(this.vslider.position().top-this.settings.wheelSpeed*c,
							this.hslider.position().left+this.settings.wheelSpeed*b,e);
					this.sliders.stop().fadeTo(this.settings.sliderOpacityTime,1);
				...
				this.bindEvent(f,'keydown',function(e){
					...
					if(vkey||a){this.easeScroll(vkey,a)}
				});
